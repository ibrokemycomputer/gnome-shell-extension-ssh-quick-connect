...
import * as PanelMenu from "resources:///org/gnome/shell/ui/panelMenu.js";
/* Part of my hacky bandaid */
import * as Mainloop from 'resource:///org/gnome/shell/mainloop.js';
/* Other */
...
class SSHQuickConnect extends PanelMenu.Button {
  fileTimer;
  constructor() {
    super._init(0.0, `${super.metadata.name} Indicator`, false);

    this.settings = super._settings.getSettings(
   ...
   */
  createIcon() {
    const iconUri = `file://${super._settings.path}/icons/icon.svg`;
  ...
  createMenu(settings = this.settings) {
    ...
    paths.forEach((path) => {
      ...
      const fileStr = decoder.decode(GLib.file_get_contents(path)[1]);
  ...
  fileListenerHack(timeout = 10) {
    this.fileTimer = Mainloop.timeout_add_seconds(timeout, () => {
      // log('hacky loop');
      this.createMenu();
...
export default class MyTestExtension extends Extension {
  ...
  enable() {
    ...
    this.indicator = new this.SSHQuickConnect();
  ...
  disable() {
    ...
    if (this.indicator.fileTimer) {
      GLib.Source.remove(this.indicator.fileTimer);
      this.indicator.fileTimer = null;
    }
  }
}
